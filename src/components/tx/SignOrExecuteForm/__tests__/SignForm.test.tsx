...
describe('SignForm', () => {
  ...
  const defaultProps = {
    onSubmit: jest.fn(),
    isOwner: true,
    txActions: { proposeTx: jest.fn(), signTx: jest.fn(), addToBatch: jest.fn(), executeTx: jest.fn() },
  ...
  it('shows a submit error', async () => {
    ...
    const { getByText } = render(
      <SignForm
        ...
        txActions={{ proposeTx: jest.fn(), signTx: mockSignTx, addToBatch: jest.fn(), executeTx: jest.fn() }}
      />,
    )
  ...
  it('submits a batch when the batch button is clicked', async () => {
    ...
    const { getByText } = render(
      <SignForm
        ...
        isBatchable
        isCreation
        txActions={{ proposeTx: jest.fn(), signTx: jest.fn(), addToBatch: mockAddToBatch, executeTx: jest.fn() }}
      />,
    )
