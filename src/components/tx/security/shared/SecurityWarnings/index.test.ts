...
describe('SecurityWarnings', () => {
  ...
  describe('mapRisk', () => {
    it('should return "Critical risk" when the severity is "HIGH', () => {
      expect(mapSeverityComponentProps[SecuritySeverity.HIGH].label).toBe('High issues')
    })

    it('should return "Low risk" when the severity is "LOW', () => {
      expect(mapSeverityComponentProps[SecuritySeverity.LOW].label).toBe('Low issues')
    })
