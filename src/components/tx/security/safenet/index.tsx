import { CircularProgress, Link, List, ListItem, ListItemIcon, ListItemText, Paper, SvgIcon, Typography } from '@mui/material'
        ...
        {guarantees.map(({ display, status, link }) => (
          <ListItem key={display}>
            <ListItemIcon>
              ...
              {status === 'pending' && (
                <SvgIcon component={Loop} inheritViewBox fontSize="small" />
              )}
            </ListItemIcon>
            <ListItemText>
              <div>
              ...
              <div className={css.pending}>
                Share this <Link href={link} target="_blank">link</Link> to the recipient to confirm the transfer
                </div>
            )}
              </ListItemText>
