...
describe('useMessageStatus', () => {
  ...
  it('should return "Awaiting confirmations" if the wallet has signed the message', () => {
  ...
  it('should return the message status if the wallet has not signed the message', () => {
    jest.spyOn(useIsMsgPendingHook, 'default').mockImplementation(() => false)
    jest.spyOn(useWalletHook, 'default').mockImplementation(() => ({ address: '0x123' } as ConnectedWallet))

    const message = {
      confirmations: [] as Message['confirmations'],
  ...
  it('should return the message status if the message is not pending', () => {
    jest.spyOn(useIsMsgPendingHook, 'default').mockImplementation(() => false)
    jest.spyOn(useWalletHook, 'default').mockImplementation(() => ({ address: '0x123' } as ConnectedWallet))
