...
describe('useShowOutreachPopup', () => {
  it('should return false if the safe is not on the targeted list', async () => {
    jest.spyOn(useIsSafeOwner, 'default').mockReturnValueOnce(true)
    jest.spyOn(useSafeAddress, 'default').mockReturnValueOnce(faker.finance.ethereumAddress())
